...
#---------------------------data generating---------------------------------- 
  ...
  ModelMultiReg <- function(n,p,pi,cor,tau){
    ...
    coeff$alpha[aInd] =c(runif(n=length(aInd)/2,min=0.6/3,max=1.2/3),  runif(n=length(aInd)/2,min=-1.2/3,max=-0.6/3))*6#runif(length(aInd),0.5,2.5) #alpha_j ~ 0.2Unif(0.5,2.5)
