...
Process <- function(n,p,pi,nonzero,nonzero_beta,q,tau,B,mode=0,matrix_width=1,correlation=corr,noise_sd=1){
...
#---------------------------DTM-----------------------------------------------
  ...
  #####################################################
  DTM=function(X,M,Y,n,p,q,pi,Index1,ratio=0.5,nonzero_beta){
    result_rej2<-rep(0,p)
    ...
    for (b in 1:B)
    {
      ...
      D_2= (1:n)[-D_1]  ##beta
      nonzero_beta=match(nonzero_beta, Index1)
  ...
  Res=DTM(Dat$X,Dat$M,Dat$Y,n,p,q,pi,B,ratio=0.5,nonzero_beta)
