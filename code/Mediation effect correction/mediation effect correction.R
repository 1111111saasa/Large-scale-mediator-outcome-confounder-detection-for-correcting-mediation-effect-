...
DTM=function(X,M,Y,n,p,q,pi,Index1,ratio=0.5){
  result_rej2<-rep(0,p)
...
DACT_DTM<-function(X,M,Y,nonbeta,nonzero_med,nonzero_conf,Index1){
  p=dim(M)[2]
  n=dim(M)[1]
  EsAB = EstimateCoef2(X,M,Y, nonbeta=nonbeta)
  ind_mediator=DACT2020(EsAB$EsAlpha[,4],EsAB$EsBeta[,4], p, q, nonzero)
  res_DTM=DTM(X,M,Y,n,p,q,pi,Index1,ratio=0.5)
  ind_confounder=res_DTM[["reject_indexDT"]]
...
truemed_DTM<-function(X,M,Y,ind_confounder,Index1){
  p=dim(M)[2]
  ...
  TNR_DTM=length(setdiff(ind_confounder,ind_confounder_H0))/max(1,length(ind_confounder_H0))
  FDR_DTM=length(setdiff(ind_confounder,ind_confounder_true))/max(1,length(ind_confounder))
  Power_DTM=length(intersect(ind_confounder,ind_confounder_true))/length(ind_confounder_true)
  
  
  
  return(list(coefficient_beta=coefficient_beta,TNR_DTM=TNR_DTM,FDR_DTM=FDR_DTM,Power_DTM=Power_DTM,ind_confounder=ind_confounder) )
}
...
coe_DACT_DTM=DACT_DTM(Dat$X,Dat$M,Dat$Y,nonbeta,nonzero_med=ind_mediator,nonzero_conf =ind_confounder ,Index1 = Index1)
coe_truemed_DTM=truemed_DTM(Dat$X,Dat$M,Dat$Y,coe_DACT_DTM$ind_confounder,Index1=Index1)
...
return(list(Dat=Dat,coefficient_alpha=coefficient_alpha,coe_DACT_DTM=coe_DACT_DTM,coe_truemed_DTM=coe_truemed_DTM,coe_oracle=coe_oracle,coe_HIMA2=coe_HIMA2_res,coe_DACT=coe_DACT,coe_eye_true=coe_eye_true,Index1=Index1))
}
