...
#---------------------------DIMOC-----------------------------------------------
  ...
  #####################################################
  DIMOC=function(X,M,Y,n,p,q,pi,Index1,B,ratio=0.5,nonzero_beta){
    result_rej2<-rep(0,p)
    ...
    for (b in 1:B)
    {
      ...
      nonzero_beta1=match(nonzero_beta, Index1)
      support_1=c(1,nonzero_beta1+1)
      ...
      #choose L1,L2
      res_tt<-  opt_L1_L2_fun(W_1,W_2,q,"",p)
      L1_L2_rej <- res_tt$W_rej_list
      estimate_DTFDR<-res_tt$estimate_DTFDR
      L1 <- L1_L2_rej[[1]]
      L2 <- L1_L2_rej[[2]]
      W_rej <- L1_L2_rej[[3]]
  ...
  Res=DIMOC(Dat$X,Dat$M,Dat$Y,n,p,q,pi,Index1,B,ratio=0.5,nonzero_beta)
